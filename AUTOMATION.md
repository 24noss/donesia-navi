...
- **飲食店ガイド（ハラール・酒類・エリア等のmapDataつき）は本パイプラインの対象外**: 既存のレストランガイド記事群（`content.config.ts`の`mapData`フィールド、料理別ガイド）はニュースではなく常時更新型のディレクトリコンテンツであり、ニュースRSSクロールでは生成できない。別途Google Places API等を使った専用の仕組みが必要（未着手）
...
- **実際のPlaces API呼び出し（本物のキーでの動作）は未検証**。新規キー発行後に`npm run discover-restaurants -- --area="Kebayoran Baru" --cuisine=japanese`のような小規模実行で確認すること
